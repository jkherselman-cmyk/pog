...
function renderHeader(){
  ...
  const links = NAV.map(([h,l])=>`<a href="${prefix}${h}" class="${h===cur?'active':''}">${l}</a>`).join('');
  const mlinks = NAV.map(([h,l])=>`<a href="${prefix}${h}">${l}</a>`).join('');
  el.innerHTML = `
    <header class="site-header">
      <div class="container">
        <a href="${prefix}index.html" class="brand">
          <img src="${prefix}assets/Logo.png" alt="POG African Safaris logo" class="brand-logo">
          <span>POG African Safaris</span>
        </a>
        <nav class="nav-desktop">${links}</nav>
        <div class="hide-mobile"><a class="btn btn-gold" href="${prefix}contact.html">Book Now</a></div>
    ...
    <div id="mobile-menu" class="mobile-menu">
      <div class="mobile-menu-head">
        <span class="brand"><img src="${prefix}assets/Logo.png" alt="" class="brand-logo"><span>POG African Safaris</span></span>
      ...
      <nav>${mlinks}</nav>
      <div class="foot"><a class="btn btn-gold" href="${prefix}contact.html">Book Now</a></div>
    </div>`;
}
      ...
      <div class="container">
        <div class="footer-grid">
          <div>
            ...
            <p style="font-size:.9rem;color:rgba(245,239,224,.7)">Where Africa's beauty, lifelong friendships, ethical hunting, and unforgettable memories
come together in the Presence of Greatness.</p>
          </div>
          <div>
            <h4>Quick Links</h4>
            <ul>${quickPrefixed}</ul>
          ...
          <div>
            ...
            <ul class="contact-list">
              <li><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg><span><a href="mailto:info@poghunting.co.za">info@poghunting.co.za</a></span></li>
              <li><span style="margin-left:32px"><b>Willie van Zyl</b></span></li>
              <li><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.13.96.37 1.9.72 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.91.35 1.85.59 2.81.72A2 2 0 0 1 22 16.92z"/></svg><span>+27 72 595 4137</span></li>
              <li><span style="margin-left:32px"><b>Marius Heinen</b></span></li>
              <li><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.13.96.37 1.9.72 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.91.35 1.85.59 2.81.72A2 2 0 0 1 22 16.92z"/></svg><span>+27 82 953 5449</span></li>
...
function initGallery(){
  ...
  grids.forEach(grid=>{
    grid.querySelectorAll('img').forEach(img=>{
      img.addEventListener('click',()=>openLightbox({type:'image',src:img.src}));
    });
    grid.querySelectorAll('.video-tile').forEach(tile=>{
      tile.addEventListener('click',()=>openLightbox({type:'video',src:tile.dataset.video}));
    });
  });
  function openLightbox(item){
    if(item.type==='video'){
      ...
      if(lbVid.src!==item.src) lbVid.src=item.src;
      lbVid.play().catch(()=>{});
    }else{
      lbVid.pause();
  ...
  if(lbVid) lbVid.addEventListener('click',e=>e.stopPropagation());
}
